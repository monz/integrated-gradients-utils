...
# from cStringIO import StringIO # not found, Python 2.x?!
...
def polarity_clip(attributions, polarity):
...
def compute_threshold_by_top_percentage(attributions,
                                        percentage=60.0,
...
def linear_transform(attributions,
    ...
    m = compute_threshold_by_top_percentage(attributions,
                                            percentage=100 - clip_above_percentile,
                                            plot_distribution=plot_distribution)
    e = compute_threshold_by_top_percentage(attributions,
...
def get_morphological_cleanup(attributions, structure=np.ones((4, 4))):
...
def get_outlines(attributions, percentage=90,
    ...
    # sorted_sums = zip(*sorted_sums_and_masks)[0]  # orig, zip(*zipped) in python3 does not work like in python2
    sorted_sums = [x for x, _ in sorted_sums_and_masks]
...
def visualize(attributions,
    ...
    if polarity == 'both':
        ...
        if overlay:
            attributions = overlay_img(attributions, image)

        return attributions
    elif polarity == 'positive':
        attributions = polarity_clip(attributions, polarity=polarity)
    ...
    if morphological_cleanup:
        attributions = get_morphological_cleanup(attributions, structure=structure)
    if outlines:
        attributions = get_outlines(attributions,
